...
export default function Admin() {
    ...
    const SECRET = "supersecretpassword123"; // Must match .env.local
    ...
    const fetchOrders = async () => {
        const res = await fetch("/api/get-orders", { headers: { "x-admin-secret": SECRET } });
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => { if (isAuth) fetchOrders(); }, 10000);
    ...
    return (
        <div style={{ maxWidth: "800px", margin: "30px auto", padding: "20px" }}>
            ...
            {orders.length === 0 ? <p style={{ backgroundColor: 'white', padding: '20px', borderRadius: '5px' }}>No new orders.</p> : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                    {orders.map(order => (
                        <div key={order.id} style={{ border: "1px solid #fcd34d", backgroundColor: "#fffbeb", padding: "20px", borderRadius: "8px" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
                                <h3 style={{ margin: 0 }}>{order.id}</h3>
                                <button onClick={() => markAsViewed(order.id)} style={{ padding: "8px 15px", backgroundColor: "#16a34a", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}>✅ Mark as Handled</button>
                            </div>
                            <p><strong>Name:</strong> {order.customer.name}</p>
                            <p><strong>Phone:</strong> {order.customer.phone}</p>
                            <p><strong>Location:</strong> {order.customer.location}</p>
                            <ul style={{ paddingLeft: '20px' }}>
                                {order.cart.map(item => <li key={item.id}>{item.name} (x{item.qty}) - <strong>GHS {item.price * item.qty}</strong></li>)}
                            </ul>
                        </div>
                    ))}
                </div>
